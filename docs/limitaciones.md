...
| L-21 | En GitHub Actions ya pasaron las pruebas, el pipeline de la muestra y su publicación en Supabase (2026-09-13). El pipeline con datos completos no ha terminado en CI: la primera ejecución se detuvo por el cambio de esquema de junio (D-037). | Tiempos con datos completos en CI siguen siendo estimaciones basadas en la ejecución local. | Ejecución manual en modo `completo` después de integrar D-037 y D-038. |
